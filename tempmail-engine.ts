...
/**
 * Robust OTP extractor tailored for Instagram, Facebook, and standard verification emails.
 * Handles:
 *  - "123 456 is your Instagram code" (spaced 6-digit with keyword after)
 *  - "123456 is your Instagram code" (contiguous 6-digit)
 *  - "Instagram security code: 123 456"
 *  - "Your confirmation code is 891-234"
 *  - Styled HTML tags: <span>123 456</span> or <font size="6">982341</font>
 *  - "FB-12345" Facebook codes
 */
export function extractOtp(subject: string, text: string, html: string): string | null {
  ...
  // 1. Spaced or hyphenated 6-digit in subject: "123 456 is your Instagram code" or "123-456"
  const sSpaced = s.match(/\b(\d{3})[-\s](\d{3})\b/);
  if (sSpaced) return sSpaced[1] + sSpaced[2];

  // 2. Contiguous 6-8 digits in subject: "123456 is your code"
  const sContig = s.match(/\b(\d{6,8})\b/);
  if (sContig) return sContig[1];

  // 3. Facebook FB-12345 pattern
  const fbMatch = all.match(/FB-(\d{5,6})/i);
  if (fbMatch) return fbMatch[1];

  // 4. Digits immediately before verification keywords (Instagram default: "123 456 is your Instagram code")
  const beforeKeyword = all.match(/\b(\d{3}[-\s]\d{3}|\d{4,8})\b(?:\s+is|\s+to|\s+for|\s+your|\s+code|\s+as|\s+security|\s+instagram|\s+facebook|\s+verification)/i);
  if (beforeKeyword) return beforeKeyword[1].replace(/[-\s]/g, '');

  // 5. Verification keyword followed by code: "security code: 123 456", "code: 123456"
  const afterKeyword = all.match(/(?:code|otp|passcode|pin|security|verification|verify|confirm|instagram|facebook)[^0-9\n]{1,40}(\b\d{3}[-\s]\d{3}\b|\b\d{4,8}\b)/i);
  if (afterKeyword) return afterKeyword[1].replace(/[-\s]/g, '');

  // 6. Any standalone spaced 6 digits in text or html body (e.g. Instagram body "349 812")
  const anySpaced = all.match(/\b(\d{3})\s+(\d{3})\b/);
  if (anySpaced) return anySpaced[1] + anySpaced[2];

  // 7. Any standalone 6-digit number
  const any6 = all.match(/\b(\d{6})\b/);
  if (any6) return any6[1];

  // 8. Standalone 4 to 8 digits
  const any4to8 = all.match(/\b(\d{4,8})\b/);
  if (any4to8) return any4to8[1];
...
export async function inboxesListMessages(address: string): Promise<TempMailMessage[]> {
  try {
    ...
    for (const m of msgs) {
      ...
      // Fetch full content to accurately extract OTP
      const full = await inboxesGetMessage(msgId);
      if (full) {
...
export async function fetchInboxMessages(token: string, address?: string): Promise<TempMailMessage[]> {
  ...
  if (token.startsWith('in:') || (addr && (addr.includes('@getairmail') || addr.includes('@getnada') || addr.includes('@inboxbear') || addr.includes('@fivermail') || addr.includes('@dropjar') || addr.includes('@robot-mail')))) {
    const cleanAddr = token.startsWith('in:') ? token.slice(3) : addr;
    liveMsgs = await inboxesListMessages(cleanAddr);
  } else if (token.startsWith('g:')) {
    liveMsgs = await guerrillaListMessages(token);
  } else if (token.startsWith('d:')) {
    liveMsgs = await dropmailListMessages(token);
  } else if (token.startsWith('mi:')) {
    liveMsgs = await mailinatorListMessages(token);
  } else if (token) {
    liveMsgs = await mailtmListMessages(token);
  } else if (addr) {
